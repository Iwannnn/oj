 ...
 * 2, skipB = 3 输出：Reference of the node with value = 8
 * 解释：第一个正式训练项目编号为 8 （注意，如果两个列表相交则不能为
 * 0）。从各自的表头开始算起，链表 A 为 [4,1,8,4,5]，链表 B 为 [5,0,1,8,4,5]。在
 ...
 * skipB = 1 输出：Reference of the node with value = 2
 * 解释：第一个正式训练项目编号为 2 （注意，如果两个列表相交则不能为
 ...
 * 输出：null
 * 解释：两套计划完全不同，返回 null。从各自的表头开始算起，链表 A 为
 * [2,6,4]，链表 B 为 [1,5]。由于这两个链表不相交，所以 intersectVal 必须为
